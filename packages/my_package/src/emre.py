...
class AStarDWAPlanner:
    ...
    def __init__(self):
        ...
        self.viz_topic = \
            f"/{self.vehicle_name}/assignment4_viz/compressed"
        ...
        # TOF
        # ==========================================
        self.use_unknown_obstacle = \
            self.cfg["bonus"]["unknown_obstacle"]
        ...
        # A*
        # ==========================================
        rospy.loginfo("Calculating Global Path with A*...")
        ...
        self.goal_tolerance = \
            self.cfg["control"]["goal_tolerance"]

        self.loop_rate = \
            self.cfg["control"]["loop_rate"]
    ...
    def run_astar(self):
        ...
        ox, oy, orad = self.static_obstacle
    ...
    def update_dynamic_obstacles(self):
        ...
        # obstacle gerçekten algılandıysa
        if 0.02 < r < self.sensing_radius:

            ox = \
                self.state[0] + \
                r * math.cos(self.state[2])

            oy = \
                self.state[1] + \
                r * math.sin(self.state[2])

            is_new = True

            for mx, my, _ in self.mapped_dynamic_obstacles:

                if self.distance((ox, oy), (mx, my)) < 0.10:
                    is_new = False
                    break

            if is_new:

                self.mapped_dynamic_obstacles.append(
                    (
                        ox,
                        oy,
                        self.cfg["obstacle"]["radius"]
                ...
                rospy.loginfo(
                    f"New obstacle mapped at: "
                    f"{ox:.2f}, {oy:.2f}"
                )

    def get_active_obstacles(self):

        obstacles = [self.static_obstacle]
    ...
    def get_lookahead_point(self, x, y):
        ...
        target_idx = min(
            ...
            closest_idx + lookahead_distance
        )
    ...
    def update_internal_pose(self, v, w, dt):
        ...
        self.state[0] += \
            v * math.cos(self.state[2]) * dt

        self.state[1] += \
            v * math.sin(self.state[2]) * dt

        self.state[2] += w * dt

        self.state[2] = \
            self.normalize_angle(self.state[2])
    ...
    def publish_cv2_visualization(
        ...
        def to_px(x, y):
            ...
            px = int(
                (x / self.map_width) * w_px
            )

            py = int(
                h_px -
                (y / self.map_height) * h_px
            )
        ...
        # Grid
        grid_step = int(
            (self.resolution / self.map_width) * w_px
        )
        ...
        for i in range(len(self.global_path) - 1):
            ...
            p1 = to_px(
                self.global_path[i][0],
                self.global_path[i][1]
            )

            p2 = to_px(
                self.global_path[i + 1][0],
                self.global_path[i + 1][1]
            )
        ...
        sense_px = int(
            (self.sensing_radius / self.map_width) * w_px
        )
        ...
        # Robot
        robot_px = int(
            (self.robot_radius / self.map_width) * w_px
        )
        ...
        hx = rx + int(
            math.cos(self.state[2]) * robot_px
        )

        hy = ry - int(
            math.sin(self.state[2]) * robot_px
        )
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            # obstacle detect edilirse hafızaya kaydet
            self.update_dynamic_obstacles()

            if self.reached_goal():

                rospy.loginfo(
                    "GOAL REACHED! Assignment 4 Complete."
                )
            ...
            control, best_trajectory, all_trajectories = \
                self.dwa_control()
            ...
            if self.cfg["simulation"]["use_internal_pose"]:
                ...
                self.update_internal_pose(
                    v,
                    w,
                    dt
                )

            self.publish_cv2_visualization(
                best_trajectory,
                all_trajectories
            )
